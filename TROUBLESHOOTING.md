...
### 2. 首次部署超时或 HTTP 502 错误

**症状**：执行命令后返回"请求超时，Overstats 服务繁忙，请稍后重试。"或 HTTP 502 错误。

**原因**：插件首次部署时，Overstats 需要初始化和优化，期间服务可能不稳定。

**解决**：
1. 等待 2 分钟，让 Overstats 完成初始化
2. 重启 Astrbot 本体（不是重启插件）
3. 再次尝试查询命令
