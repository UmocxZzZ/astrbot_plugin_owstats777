...
class DashenMatchModule:
    ...
    async def query_match_detail_court(
        self,
        *,
        query: Optional[DashenMatchQuery] = None,
        customer_token: str = "",
        ...
        if detail.match_kind == "fight":
            replies.append(_text_reply("角斗对局暂不支持 AI 开庭。"))
            return DashenMatchRepliesOutput(
                customer_token=customer_token,
                resolved_bnet=resolved_bnet,
                replies=replies,
        ...
        court_result = await self._build_ai_court(
            match_data=detail_root,
            all_player_details=player_details,
            target_id=target_id,
        )
        if court_result.get("json") is not None:
            json_data = dict(court_result["json"])
            json_data["generated_at"] = time.strftime("%Y-%m-%d %H:%M", time.localtime())
            court_image = render_court_report(
                json_data,
                target_hero_images=build_target_hero_icons(focus_detail["heroList"], size=40),
                map_name=map_name_for_match(detail_root),
                map_icon_img=map_icon_image_for_match(detail_root),
                match_result=self._match_result_text(detail_root),
                footer_source=court_result.get("footer_source"),
            )
            replies.append(_image_reply(court_image))
        else:
            replies.append(_text_reply(court_result.get("fallback_text") or "AI开庭暂不可用。"))
...
""".strip()
    ...
    async def _build_ai_court(
        ...
        *,
        ...
        detailed_text = generate_detailed_stats_text(all_player_details, target_id)
        score_bundle = calculate_match_scores(match_data)
        persona_prompt = str(getattr(app_config, "ANALYSIS_PERSONA_COURT", "") or "").strip()
        if persona_prompt:
            persona_prompt = persona_prompt.format(
                target_id=target_id,
                anti_pressure=score_bundle["anti_pressure"],
...
【身份】
你是电竞法庭的主审法官。本庭今日审理的是一场守望先锋对局。你需要以绝对中立的视角，基于数据证据，做出公正判决。
...
5. 对焦点玩家所在队伍的所有玩家（含焦点玩家自己）逐一做出有功/有过/无功无过的判决，附一句话理由。注意：5v5 模式为 5 人，6v6 模式为 6 人。
...
{{
  ...
  "player_verdicts": [
    {{"player_id": "玩家1", "verdict": "有功", "reason": "一句话理由"}},
    {{"player_id": "玩家2", "verdict": "有过", "reason": "一句话理由"}},
    {{"player_id": "玩家3", "verdict": "无功无过", "reason": "一句话理由"}}
  ],
  "role_comparison": [
    "坦克位：[双方对比结论]",
    "输出位：[双方对比结论]",
...
""".strip()
        ...
        if not base_url or not api_key:
            return {"fallback_text": "AI开庭未配置 ANALYSIS_BASE_URL / ANALYSIS_API_KEY。"}
        ...
        try:
            ...
            return {"fallback_text": f"AI开庭生成失败。错误：invalid json ({model})"}
        except Exception as exc:
            return {"fallback_text": f"AI开庭生成失败。错误：{type(exc).__name__}: {exc}"}
