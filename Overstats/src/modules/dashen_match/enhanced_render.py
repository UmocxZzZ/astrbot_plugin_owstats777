...
def render_court_report(
    ...
    *,
    ...
    # 解析 MVP 和最差玩家
    mvp = json_data.get("mvp", {})
    worst = json_data.get("worst", {})
    mvp_text = f"{mvp.get('player_id', '未知')} - {mvp.get('reason', '')}" if mvp else "未知"
    worst_text = f"{worst.get('player_id', '未知')} - {worst.get('reason', '')}" if worst else "未知"

    # 解析原罪清单
    sins = json_data.get("sins", [])
    sins_text = "\n".join([f"  {i+1}. {sin}" for i, sin in enumerate(sins)]) if sins else "无"

    # 解析全场判决
    player_verdicts = json_data.get("player_verdicts", [])
    verdict_colors = {"有功": (39, 174, 96, 255), "有过": (235, 87, 87, 255), "无功无过": (156, 166, 182, 255)}
    verdict_lines = []
    for pv in player_verdicts:
        vid = pv.get("player_id", "未知")
        vverdict = pv.get("verdict", "无功无过")
        vreason = pv.get("reason", "")
        verdict_lines.append({"player_id": vid, "verdict": vverdict, "reason": vreason})

    # 解析三路对比
    role_comparison = json_data.get("role_comparison", [])
    role_text = "\n".join([f"  - {item}" for item in role_comparison]) if isinstance(role_comparison, list) else str(role_comparison)

    sections = [
        {"type": "score", "title": "法官判决", "id": json_data.get("player_id", "未知"), "score": json_data.get("score", "N/A"), "verdict": json_data.get("verdict", "")},
        {"type": "text", "title": "本局 MVP", "text": mvp_text},
        {"type": "text", "title": "本局被告", "text": worst_text},
        {"type": "text", "title": "原罪清单", "text": sins_text},
        {"type": "verdicts", "title": "全场判决", "verdicts": verdict_lines},
        {"type": "text", "title": "三路对位对比", "text": role_text},
        {"type": "text", "title": "关键转折点", "text": json_data.get("key_moment", "")},
        {"type": "carry_index", "title": "全场表现评分", "data": json_data.get("carry_index_data", [])},
        {"type": "attributes", "title": "团队属性评分", "attr": json_data.get("attribute_scores", {}), "evaluation": ""},
        {"type": "text", "title": "法官结案陈词", "text": json_data.get("closing", "")},
    ]
    ...
    for section in sections:
        ...
        if section["type"] == "score":
            ...
            total_height += len(wrap_lines(section["text"], font_text, width - padding * 2 - 20)) * (22 + line_spacing) + 15
        elif section["type"] == "verdicts":
            reason_max_w = width - padding - 320 - 20
            for pv in section["verdicts"]:
                vreason = pv.get("reason", "")
                lines = wrap_lines(vreason, font_text, reason_max_w)
                total_height += max(1, len(lines)) * (22 + line_spacing) + 8
            total_height += 15
        elif section["type"] == "carry_index":
            total_height += len(section["data"]) * 35 + 15
        elif section["type"] == "attributes":
    ...
    if map_icon_img is not None:
        ...
        draw.rectangle([0, 0, width, 80], fill=(180, 30, 30, 255))

    draw.text((padding, 18), ">> AI开庭", font=font_title, fill=(255, 255, 255, 255))
    header_text = f"{map_name or '本场对局'} - {match_result or '总览'}"
    draw.text((width - padding - measure(header_text, _font_chinese(22)), 27), header_text, font=_font_chinese(22), fill=(255, 255, 255, 220))
    ...
    for section in sections:
        ...
        if section["type"] == "score":
            ...
            if verdict:
                for line in wrap_lines(verdict, font_text, width - padding * 2 - 20):
                    draw.text((padding + 20, current_y), line, font=font_text, fill=(255, 200, 100, 255))
                    current_y += 22 + line_spacing
            current_y += 15
        elif section["type"] == "text":
            for line in wrap_lines(section["text"], font_text, width - padding * 2 - 20):
            ...
            for pv in section["verdicts"]:
                ...
                vverdict = pv.get("verdict", "无功无过")
                vreason = pv.get("reason", "")
                vcolor = verdict_colors.get(vverdict, (220, 225, 230, 255))
                draw.text((padding + 20, current_y), f"[{vverdict}]", font=font_text, fill=vcolor)
                draw.text((padding + 120, current_y), f"{vid}", font=font_text, fill=(220, 225, 230, 255))
                # reason 文本自适应换行
                reason_max_w = width - padding - 320 - 20
                for line in wrap_lines(vreason, font_text, reason_max_w):
                    draw.text((padding + 320, current_y), line, font=font_text, fill=(180, 185, 190, 255))
                    current_y += 22 + line_spacing
                current_y += 8
            current_y += 15
        elif section["type"] == "carry_index":
