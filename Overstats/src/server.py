...
class OverstatsCoreService:
    ...
    async def handle_dashen_match_detail_court(self, payload: Dict[str, object]) -> Dict[str, object]:
        return await self.dashen_request_queue.run(
            "match_detail_court",
            lambda: self._handle_dashen_match_detail_court(payload),
        )
    ...
    async def _handle_dashen_match_detail_court(self, payload: Dict[str, object]) -> Dict[str, object]:
        bnet_id = str(payload.get("bnet_id") or payload.get("bnetId") or "").strip()
        customer_token = str(payload.get("customer_token") or payload.get("customerToken") or "").strip()
        match_id = str(payload.get("match_id") or payload.get("matchId") or "").strip()
        index_value = payload.get("index")
        ...
        result = await dashen_match_module.query_match_detail_court(
            query=query,
            customer_token=customer_token,
            match_id=match_id,
            index=int(index_value) if index_value is not None else None,
...
def create_server(config: APIConfig) -> ThreadingHTTPServer:
    ...
    class OverstatsRequestHandler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:
            ...
            if path == "/api/v2/dashen-match/detail/court":
                self._handle_dashen_match_detail_court_post()
                return
        ...
        def _handle_dashen_match_detail_court_post(self) -> None:
            try:
                payload = self._read_json_body()
            except ValueError as exc:
                self._send_json(
            ...
            try:
                result = async_runner.run(service.handle_dashen_match_detail_court(payload))
            except ModuleError as exc:
                self._send_json(
                    HTTPStatus(exc.status_code),
                    {
